...
export function useCheckCustomerCredit() {
  const { profile } = useAuth();
  return useMutation({
    mutationFn: async ({ customer_id, order_total }: { customer_id: string; order_total: number }) => {
...
export function useConfirmSalesOrder() {
  ...
  return useMutation({
    mutationFn: async (order_id: string) => {
      const { data, error } = await supabase.rpc("confirm_sales_order", { _order_id: order_id });
      if (error) throw error;
      return data as any;
    },
    onSuccess: (res: any) => {
...
export interface CreditNoteInput {
  invoice_id: string;
  cn_date: string;
  reason: "return" | "pricing" | "discount" | "cancellation";
  notes?: string;
...
export function useCreditNotes() {
  const { profile } = useAuth();
  return useQuery({
    queryKey: ["sales", "credit_notes", profile?.company_id],
    enabled: !!profile?.company_id,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("credit_notes")
...
export function useCreateCreditNote() {
  ...
  const { profile } = useAuth();
  return useMutation({
    mutationFn: async (input: CreditNoteInput) => {
      const companyId = profile!.company_id!;
      const subtotal = input.lines.reduce((s, l) => s + l.qty * l.rate, 0);
      const tax_amount = input.lines.reduce((s, l) => s + l.qty * l.rate * (l.tax_percent / 100), 0);
      const total = subtotal + tax_amount;
      const cn_number = await nextDocNumber(companyId, "CN", "CN");
      const { data: cn, error } = await supabase
        ...
        .insert({
          ...
          gst_adjustment: Math.round(tax_amount * 100) / 100,
          notes: input.notes ?? null,
          created_by: profile!.id,
        })
        .select("id")
      ...
      if (input.lines.length) {
        const rows = input.lines.map((l, i) => ({
          ...
          rate: l.rate,
          tax_percent: l.tax_percent,
          line_total: Math.round((l.qty * l.rate * (1 + l.tax_percent / 100)) * 100) / 100,
          position: i,
        }));
        const { error: e2 } = await supabase.from("credit_note_items").insert(rows);
        if (e2) throw e2;
      }
      return cn.id;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["sales", "credit_notes"] });
      toast.success("Credit note created");
    },
    onError: (e: any) => toast.error(e?.message ?? "Failed to create credit note"),
  });
}
