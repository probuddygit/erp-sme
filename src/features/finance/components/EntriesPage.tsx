...
interface Props {
  ...
  showMode?: boolean;
  loading?: boolean;
  entryType?: keyof typeof ENTRY_META;
}

const PAGE_SIZE = 8;
const DEFAULT_STATUSES = ["draft", "pending", "approved", "posted", "rejected", "cancelled"];

export function EntriesPage({ title, description, icon: Icon, data, statuses, showParty, showMode, loading, entryType }: Props) {
  ...
  const short = ENTRY_META[entryType ?? data[0]?.type ?? "journal"].short;
      ...
      <Card>
        <CardContent className="space-y-3 p-4">
          <FilterBar
            ...
            actions={
              <>
                <Button size="sm" variant="outline" onClick={() => exportEntries(title, filtered)}>
                  <Download className="mr-1.5 h-4 w-4" />Export
                </Button>
                <Button size="sm" disabled title="Vouchers are generated by the source documents"><Plus className="mr-1.5 h-4 w-4" />New {short}</Button>
              </>
            }
          />
          ...
          <div className="overflow-x-auto rounded-lg border border-border">
            <Table>
              ...
              <TableBody>
                {loading ? (
                  <TableRow>
                    <TableCell colSpan={9} className="py-10 text-center text-muted-foreground">Loading ledger postings…</TableCell>
                  </TableRow>
                ) : pageData.length === 0 ? (
                  <TableRow>
