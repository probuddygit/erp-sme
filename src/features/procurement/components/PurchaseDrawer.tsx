...
import {
  FileText, Paperclip, MessageSquare, Activity, ShieldCheck, History,
  Upload, Download, CheckCircle2, XCircle, Send, Printer, Mail,
} from "lucide-react";
import {
  APPROVAL_TONES, STATUS_TONES, formatDate, formatDateTime, formatINR,
  type Attachment, type Comment, type PurchaseTx, DOC_META,
...
export function PurchaseDrawer({ transaction, open, onOpenChange }: Props) {
      ...
      <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
        <SheetHeader className="space-y-1">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <SheetTitle className="truncate text-xl">{meta.label} · {transaction.number}</SheetTitle>
              <SheetDescription>{transaction.vendor} · {formatDate(transaction.date)}</SheetDescription>
            </div>
            <div className="flex flex-col items-end gap-1.5 shrink-0">
              <StatusBadge label={transaction.status.replace(/_/g, " ")} tone={STATUS_TONES[transaction.status]} />
              <StatusBadge label={`Approval: ${transaction.approvalStatus}`} tone={APPROVAL_TONES[transaction.approvalStatus]} />
            </div>
          </div>
          <div className="mt-3 flex flex-wrap gap-1.5">
            ...
            <Button size="sm" variant="outline"><Send className="mr-1.5 h-3.5 w-3.5" />Send to vendor</Button>
          </div>
        </SheetHeader>
        ...
        <Tabs defaultValue="details" className="mt-5">
          ...
          <TabsContent value="files" className="mt-4 space-y-3">
            ...
            {attachments.length === 0 ? <EmptyState message="No attachments yet." /> : (
          ...
          <TabsContent value="approval" className="mt-4 space-y-4">
            ...
            <div className="rounded-lg border border-border bg-card p-4">
              ...
              <ol className="mt-3 space-y-3">
                {["Raised by buyer", "Procurement Head review", "Finance sign-off (if > ₹2L)"].map((step, i) => (
                  <li key={step} className="flex items-start gap-2 text-sm">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {i === 0 ? transaction.buyer : i === 1 ? transaction.approver ?? "Pending" : "Auto-triggered"}
                      </div>
                    </div>
                  </li>
                ))}
          ...
          <TabsContent value="timeline" className="mt-4">
            {transaction.timeline.length === 0 ? <EmptyState message="No activity yet." /> : (
          ...
          <TabsContent value="audit" className="mt-4">
            {transaction.audit.length === 0 ? <EmptyState message="No audit entries yet." /> : (
...
function HeaderBlock({ tx }: { tx: PurchaseTx }) {
  const rows: { label: string; value: string }[] = [
    { label: "Document #", value: tx.number },
    { label: "Date", value: formatDate(tx.date) },
    { label: "Vendor", value: `${tx.vendor} (${tx.vendorCode})` },
    { label: "GSTIN", value: tx.gstin ?? "—" },
    { label: "Buyer", value: tx.buyer },
    { label: "Reference", value: tx.reference ?? "—" },
  ];
  if (tx.warehouse)    rows.push({ label: "Warehouse",     value: tx.warehouse });
  if (tx.requiredBy)   rows.push({ label: "Required By",   value: formatDate(tx.requiredBy) });
  if (tx.validUntil)   rows.push({ label: "Valid Until",   value: formatDate(tx.validUntil) });
  if (tx.expectedDate) rows.push({ label: "Expected Date", value: formatDate(tx.expectedDate) });
  if (tx.receivedDate) rows.push({ label: "Received Date", value: formatDate(tx.receivedDate) });
  ...
  if (tx.reason)       rows.push({ label: "Reason",        value: tx.reason });
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <AddressBox label="Vendor address" value={tx.billingAddress} />
        <AddressBox label="Ship-to address" value={tx.shippingAddress} />
      </div>
    </div>
  );
}
...
function ItemsGrid({ tx }: { tx: PurchaseTx }) {
  return (
    <div className="overflow-hidden rounded-lg border border-border">
      <div className="border-b border-border bg-muted/50 px-3 py-2 text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Items</div>
...
function TaxesBlock({ tx }: { tx: PurchaseTx }) {
  return (
    <div className="grid gap-3 md:grid-cols-2">
      <div className="rounded-lg border border-border bg-card p-4">
      ...
      <div className="rounded-lg border border-border bg-card p-4">
        ...
        <div className="mt-2 space-y-1.5 text-sm">
          ...
          <div className="mt-2 flex items-center justify-between border-t border-border pt-2 text-base font-semibold">
            <span>{tx.docType === "vendor_payment" ? "Payment amount" : "Grand total"}</span>
            <span>{formatINR(tx.paymentAmount ?? tx.grandTotal)}</span>
          </div>
        </div>
      </div>
    </div>
