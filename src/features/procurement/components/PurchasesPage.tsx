...
import {
  APPROVAL_TONES, DOC_META, STATUS_TONES, formatDate, formatINR,
  type ApprovalStatus, type PurchaseTx,
} from "@/features/procurement/data";
import { StatusBadge } from "@/features/sales/components/StatusBadge";
import { PurchaseDrawer } from "@/features/procurement/components/PurchaseDrawer";
...
interface Props {
  ...
  data: PurchaseTx[];
  statuses: string[];
  extraColumns?: { header: string; render: (t: PurchaseTx) => React.ReactNode }[];
...
export function PurchasesPage({
  title, description, icon: Icon, data, statuses, extraColumns = [], kpi,
}: Props) {
  ...
  const [selected, setSelected] = useState<PurchaseTx | null>(null);

  const buyers = useMemo(() => Array.from(new Set(data.map((t) => t.buyer))), [data]);
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((t) => {
      ...
      if (!term) return true;
      return [t.number, t.vendor, t.reference ?? "", t.buyer].some((v) => v.toLowerCase().includes(term));
    });
  }, [data, search, status, approval, buyer]);

  const pageCount = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const pageData = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);

  const totals = useMemo(() => {
    const total = filtered.reduce((s, t) => s + (t.paymentAmount ?? t.grandTotal), 0);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardContent className="space-y-3 p-4">
          <FilterBar
            ...
            placeholder={`Search ${title.toLowerCase()} by #, vendor, reference…`}
            filters={[
              ...
              { key: "buyer", label: "Buyer", value: buyer,
                onChange: (v) => { setBuyer(v); setPage(1); },
                options: buyers.map((b) => ({ value: b, label: b })) },
            ]}
            actions={
              <>
                <Button size="sm" variant="outline"><Download className="mr-1.5 h-4 w-4" /> Export</Button>
                <Button size="sm"><Plus className="mr-1.5 h-4 w-4" /> New {DOC_META[data[0]?.docType]?.short ?? "record"}</Button>
              </>
            }
          />
                  ...
                  pageData.map((t) => (
                    <TableRow key={t.id} className="cursor-pointer hover:bg-muted/50" onClick={() => setSelected(t)}>
                      ...
                      <TableCell>
                        ...
                        <div className="text-xs text-muted-foreground">{t.vendorCode}</div>
                      </TableCell>
                      ...
                      {extraColumns.map((c) => (
                        <TableCell key={c.header} className="text-sm">{c.render(t)}</TableCell>
                      ))}
                      ...
                      <TableCell className="text-center text-xs text-muted-foreground">
                        <span className="inline-flex items-center gap-2">
                          <span className="inline-flex items-center gap-0.5"><Paperclip className="h-3 w-3" />{t.attachments.length}</span>
                          <span className="inline-flex items-center gap-0.5"><MessageSquare className="h-3 w-3" />{t.comments.length}</span>
                        </span>
                      </TableCell>
                    </TableRow>
                  ))
      ...
      <PurchaseDrawer transaction={selected} open={!!selected} onOpenChange={(o) => !o && setSelected(null)} />
