...
import { Lock, ShieldCheck, LayoutDashboard, ClipboardCheck, AlertTriangle, BarChart3 } from "lucide-react";
...
const TABS: Tab[] = [
  { to: "/app/quality", label: "Overview", icon: LayoutDashboard, exact: true },
  { to: "/app/quality/inspections", label: "Inspections", icon: ClipboardCheck },
  { to: "/app/quality/ncr", label: "NCRs", icon: AlertTriangle },
  { to: "/app/quality/reports", label: "Reports", icon: BarChart3 },
];
...
function QualityLayout() {
  ...
  if (!hasModule("quality")) {
    return (
      <Empty title="Quality is disabled" text={`This module isn't enabled for ${company?.name ?? "your company"}.`}>
        <Button asChild><Link to="/app">Back to dashboard</Link></Button>
      </Empty>
    );
  }
  if (!canAccessModule("quality")) {
    return <Empty title="Access denied" text="Your role doesn't include the Quality module." />;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Incoming, in-process &amp; finished goods inspections, NCR &amp; batch traceability.</p>
        </div>
      </div>
