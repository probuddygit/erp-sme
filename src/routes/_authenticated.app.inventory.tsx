...
import { Lock, Boxes, LayoutDashboard, Package, Warehouse, ArrowLeftRight } from "lucide-react";
...
function InventoryLayout() {
  ...
  if (!hasModule("inventory")) {
    return (
      <Empty title="Inventory is disabled" text={`This module isn't enabled for ${company?.name ?? "your company"}.`}>
        <Button asChild><Link to="/app">Back to dashboard</Link></Button>
      </Empty>
    );
  }
  if (!canAccessModule("inventory")) {
    return <Empty title="Access denied" text="Your role doesn't include the Inventory module." />;
  }
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Items, warehouses, batches, stock movements &amp; valuation.
          </p>
        </div>
      </div>
